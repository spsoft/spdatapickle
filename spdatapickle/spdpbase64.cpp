...
SP_DPBase64EncodedBuffer :: SP_DPBase64EncodedBuffer( const void * plainBuffer, size_t len )
{
	mLength = len * 4 / 3 + 16;
...
SP_DPBase64EncodedBuffer :: ~SP_DPBase64EncodedBuffer()
{
	free( mBuffer );
	mBuffer = NULL;
}

const char * SP_DPBase64EncodedBuffer :: getBuffer()
{
	return mBuffer;
}

size_t SP_DPBase64EncodedBuffer :: getLength()
{
	return mLength;
}
...
SP_DPBase64DecodedBuffer :: SP_DPBase64DecodedBuffer( const char * encodedBuffer, size_t len )
{
	mLength = len * 3 / 4 + 16;
	mBuffer = (char*)malloc( mLength );
...
SP_DPBase64DecodedBuffer :: ~SP_DPBase64DecodedBuffer()
{
	free( mBuffer );
	mBuffer = NULL;
}

const void * SP_DPBase64DecodedBuffer :: getBuffer()
{
	return mBuffer;
}

size_t SP_DPBase64DecodedBuffer :: getLength()
{
	return mLength;
}
